                ...
                currentItems.map((item) => (
                    <div className='p-4 d-block d-lg-none bg-dark bg-opacity-75 mb-3 '>
                        <div class="d-flex  justify-content-between mb-3">
                            ...
                            {item.Status == "Published" ? <button className="btn btn-warning text-light rounded-4">{item.Status}</button> : <button className="btn  btn-gray  text-light rounded-4">{item.Status}</button>}
                        </div>
