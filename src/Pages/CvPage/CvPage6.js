...
import Form6 from '../../Components/CV/Form6'
...
function CvPage6() {
    ...
    const initialValues = {
      ...
      const addForm = () => {
        const newForm = {
          id: formik.values.forms6.length + 1,
          inputs: initialValues.forms6[0].inputs.map((input) => ({
            ...input,
            value: "",
          })),
        };
        formik.setValues({
          ...formik.values,
          forms6: [...formik.values.forms6, newForm],
        });
      };
    
      const removeForm = (id) => {
        const updatedForms = formik.values.forms6.filter((form) => form.id !== id);
        formik.setValues({
          ...formik.values,
          forms6: updatedForms,
        });
      ...
      const handleInputChange = (formIndex, inputIndex, value) => {
        const updatedForms = [...formik.values.forms6];
...
return (
    ...
    <form   className="col-12 col-lg-6 me-lg-5">
    <h3>Honers & awards</h3>
    {formik.values.forms6.map((form, formIndex) => (
      <div 
        key={form.id}
      ...
      >
        {form.inputs.map((input, inputIndex) => (
          <div key={input.id} style={{ marginBottom: "10px" }}>
            <h5 htmlFor={`forms6.${formIndex}.inputs.${inputIndex}.value`}>
              {input.label}
            </h5>
            {input.label == 'Description'?<textarea
            type="text"
            id={`forms6.${formIndex}.inputs.${inputIndex}.value`}
            value={input.value}
            onChange={(e) =>
    ...
    <div className="container btns-form-part">
            <div className="row my-3">
                <div className="col-12 col-lg-2 mb-3 mb-lg-0">
                    <button type="submit" onClick={formik.handleSubmit} className="btn bg-gold text-white text-uppercase btn-gold-hover py-3 w-100">Continue</button>
                </div>
