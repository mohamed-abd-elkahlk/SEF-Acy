...
return (
    ...
    {formik.values.forms5.map((form, formIndex) => (
      <div
        key={form.id}
        className="rounded-3 p-3 bg-black text-white my-2"
        style={{ border: "2px dashed #BF9B30" }}
      >
        {form.inputs.map((input, inputIndex) => (
          <div key={input.id} style={{ marginBottom: "10px" }}>
            <h5 htmlFor={`forms5.${formIndex}.inputs.${inputIndex}.value`}>
              {input.label}
            </h5>
            <input
              type="text"
              id={`forms5.${formIndex}.inputs.${inputIndex}.value`}
              value={input.value}
              onChange={(e) =>
    ...
    <div className="col-12 col-lg-2 mb-3 mb-lg-0">
        <button type="submit" onClick={ () => (formik.handleSubmit(), Navigate('/CV/Form5'))} className="btn bg-gold text-white text-uppercase btn-gold-hover py-3 w-100">Continue</button>
    </div>
